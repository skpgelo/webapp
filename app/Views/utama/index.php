...
<section class="section">
  <div class="card">
    <div class="card-header"><h4>Data Pegawai</h4>
    <button class="btn btn-primary" id="btn-tambah">+ Tambah</button>
      <div class="mb-3 row">
          <div class="col-md-6">
              <select id="filter_kecamatan" class="form-select form-select-sm d-inline w-auto">
                  <option value="">Semua Kecamatan</option>
                  <option value="Cikarang">Cikarang</option>
                  <option value="Cibitung">Cibitung</option>
                  <option value="Bekasi">Bekasi</option>
              </select>
              <select id="filter_kategori" class="form-select form-select-sm d-inline w-auto">
              ...
              <div class="mb-3 row">
                <div class="col-md-8">
                    ...
                    <input type="date" id="filter_tgl_awal" class="form-control form-control-sm d-inline w-auto">
                    <span class="mx-1">s/d</span>
                    <input type="date" id="filter_tgl_akhir" class="form-control form-control-sm d-inline w-auto">
                    <button id="btnFilter" class="btn btn-sm btn-primary">Filter</button>
                    <button id="btnReset" class="btn btn-sm btn-secondary">Reset</button>
                </div>
                <div class="col-md-4 text-end">
                  <a id="btnExcel" href="<?= site_url('utama/export-excel') ?>" class="btn btn-success btn-sm"><i class="fas fa-file-excel"></i> Excel</a>
                  <a id="btnPdf" href="<?= site_url('utama/export-pdf') ?>" class="btn btn-danger btn-sm"><i class="fas fa-file-pdf"></i> PDF</a>                
                </div>
              </div>
            </div>
          <div class="col-md-6 text-end">
              <a id="btnExcel" href="<?= site_url('utama/export-excel') ?>" class="btn btn-success btn-sm">
                  <i class="fas fa-file-excel"></i> Export Excel
              </a>
              <a id="btnPdf" href="<?= site_url('utama/export-pdf') ?>" class="btn btn-danger btn-sm">
                  <i class="fas fa-file-pdf"></i> Export PDF
              </a>
              <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalForm">Tambah Data</button>
          </div>
      </div>
    </div>
    <div class="card-body">
<table id="table" class="table table-striped" width="100%">
  <thead><tr>
    ...
    <th>Alamat</th>
    <th>Preview</th>
    <th>Aksi</th> <!-- ini sekarang isi: Download PDF, Download Image, Detail, Edit, Hapus -->
  </tr></thead>
</table>
    </div>
  </div>
...
<?= $this->section('script')?>
<script>
let table, kategoriOpt='';
$(function(){
  // table = $('#table').DataTable({
  //   processing:true,serverSide:true,
  //   ajax:{url:"<?=site_url('utama/list')?>",
  //     type:"POST"},
...
$('#table').DataTable({
  ...
  $('#btnFilter').click(function(){ 
      table.ajax.reload();
      updateExportLink(); // update link export sesuai filter
  });

  function updateExportLink(){
      let kec = $('#filter_kecamatan').val();
      let kat = $('#filter_kategori').val();
      let query = '?kecamatan='+encodeURIComponent(kec)+'&kategori='+encodeURIComponent(kat);
      $('#btnExcel').attr('href', '<?= site_url('utama/export-excel') ?>'+query);
      $('#btnPdf').attr('href', '<?= site_url('utama/export-pdf') ?>'+query);
  }
  updateExportLink(); // jalanin pertama kali
